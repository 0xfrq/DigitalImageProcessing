...
class FaceFeatureExtractor:
    ...
    def extract_features(self, face_rgb_100, eyes_rects_100):
        ...
        eye_area_skin = skin_mask[0:50, 0:100]
        eye_area_edge = edges[0:50, 0:100]
        eye_non_skin_ratio = float(np.sum(eye_area_skin == 0)) / 5000.0
        eye_edge_density = float(np.sum(eye_area_edge > 50)) / 5000.0
        
        nose_region_edge = edges[40:75, 35:65]
        nose_edge_density = float(np.sum(nose_region_edge > 50)) / (30*35)
        
        mouth_region_skin = skin_mask[70:95, 25:75]
        mouth_non_skin_ratio = float(np.sum(mouth_region_skin == 0)) / (50*25)
